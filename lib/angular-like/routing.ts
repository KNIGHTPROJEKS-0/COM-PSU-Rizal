...
export class RouterService {
  ...
  private routeParams$ = new BehaviorSubject<Record<string, string>>({});
  private queryParams$ = new BehaviorSubject<Record<string, string>>({});
  private navigation$ = new Subject<{ from: string; to: string }>();

  constructor() {
    if (typeof window !== 'undefined') {
      this.currentRoute$.next(window.location.pathname);
  ...
  navigate(route: string, params?: Record<string, string>, queryParams?: Record<string, string>): void {
    ...
    this.navigation$.next({ from, to: newRoute });
  ...
  navigateByUrl(url: string): void {
    ...
    this.navigation$.next({ from, to: url });
